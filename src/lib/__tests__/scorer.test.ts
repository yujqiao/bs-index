...
describe("getVerdict", () => {
  it("returns green for negative scores", () => {
    expect(getVerdict(-5).label).toBe("Seems Legit");
  });

  it("returns yellow for mild BS", () => {
    expect(getVerdict(20).label).toBe("Mild BS Detected");
  });

  it("returns orange for significant BS", () => {
    expect(getVerdict(50).label).toBe("Significant BS");
  });

  it("returns red for major BS", () => {
    expect(getVerdict(100).label).toBe("Major BS Alert");
  });
